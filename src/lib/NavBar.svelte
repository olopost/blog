<script lang="ts">
    ...
    export function search(evt) {
        if (evt.key == "Enter") {
            ...
            const url = buildUrl('/', {'search': item});
            goto(url, {});
        }
    }
</script>
<nav>
  ...
  <div class="relative mt-2 flex items-center">
    <input on:keypress={search} type="text" name="search" id="search" class="block w-full rounded-md border-0 py-1.5 pr-14 text-gray-900 shadow-sm ring-1 ring-inset ring-gray-300 placeholder:text-gray-400 focus:ring-2 focus:ring-inset focus:ring-indigo-600 sm:text-sm sm:leading-6">
